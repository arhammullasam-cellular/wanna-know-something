...
async function leaveRoomOnline() {
  const room = APP_STATE.room;
  if (!room || !APP_STATE.user || !APP_STATE.db) {
    clearRoomState();
    showScreen('home');
    return;
  }
  ...
  try {
    if (APP_STATE.role === 'guest' && room.guestId === APP_STATE.user.uid) {
      await updateDoc(doc(APP_STATE.db, 'rooms', room.code), {
        guestId: null,
        guestName: null,
        lastActivityAt: serverTimestamp()
      });
    } else if (APP_STATE.role === 'host') {
      await updateDoc(doc(APP_STATE.db, 'rooms', room.code), {
        state: 'finished',
        lastActivityAt: serverTimestamp()
      });
...
function wireUI() {
  ...
  // Navigation: use qs for selectors, never $.
  onAll('[data-nav="home"]', 'click', event => {
    event.preventDefault();
    clearRoomState();
    showScreen('home');
  });

  onAll('[data-back]', 'click', button => {
  ...
  on('leave-room', 'click', () => {
    if (APP_STATE.mode === 'online') leaveRoomOnline();
    else {
      clearRoomState();
      showScreen('home');
    }
